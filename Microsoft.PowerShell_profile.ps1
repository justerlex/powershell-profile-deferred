...
# ═══════════════════════════════════════════════════════════════════════════════
#  DEFERRED — runs via OnIdle after the prompt appears
#
#  Unlike the old wrapper, this uses PowerShell.OnIdle which fires in the MAIN
#  session state. No background runspace, no SessionState injection, no
#  reflection hacks. Terminal-Icons formatters propagate correctly because
#  everything runs in the same runspace.
# ═══════════════════════════════════════════════════════════════════════════════
...
$null = Register-EngineEvent -SourceIdentifier PowerShell.OnIdle -MaxTriggerCount 1 -Action {
    ...
    function global:Invoke-Profile {
    ...
    function global:sed($file, $find, $replace) {
        (Get-Content $file).replace("$find", $replace) | Set-Content $file
    }

    function global:which($name) {
        Get-Command $name | Select-Object -ExpandProperty Definition
    }

    function global:export($name, $value) {
        set-item -force -path "env:$name" -value $value;
    }

    function global:pkill($name) {
        Get-Process $name -ErrorAction SilentlyContinue | Stop-Process
    }

    function global:pgrep($name) {
        Get-Process $name
    }
    ...
    $customProfile = Join-Path $profileDir "profile.ps1"
